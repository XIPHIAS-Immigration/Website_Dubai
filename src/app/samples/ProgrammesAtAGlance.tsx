...
export default function ProgrammesAtAGlance({ serifClass }: { serifClass: string }) {
  const [tab, setTab] = useState<"citizenship" | "residency">("citizenship");
  const rows = DATA[tab];
  return (
    <main>
      <section className="relative isolate px-6 py-24 text-[#0c1f3f] sm:px-12 lg:px-20" style={{ background: "#f3f7fd" }}>
        <Badge>Info section · Programmes at a glance (real data)</Badge>
        <div className="mx-auto max-w-6xl">
            ...
            {TABS.map((t) => {
              ...
              return (
                <button key={t.key} onClick={() => setTab(t.key)} className="relative px-1 pb-3 pr-7 text-left text-[14px] font-semibold transition-colors" style={{ color: on ? INK : `${INK}66` }}>
                  {t.label}
                  {on ? <motion.span layoutId="tabline" className="absolute -bottom-px left-0 h-0.5 w-full" style={{ background: GOLD }} /> : null}
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-2 overflow-x-auto">
            <div className="min-w-[760px]">
              ...
              {rows.map((r, i) => (
                ...
                >
                  <span aria-hidden className="absolute left-0 hidden h-8 w-0.5 origin-top scale-y-0 transition-transform duration-300 group-hover:scale-y-100" style={{ background: GOLD }} />
          ...
          <div className="mt-7 flex flex-wrap items-center justify-between gap-4">
            ...
            <a href="#" className="group inline-flex items-center gap-2 text-[13px] font-semibold uppercase tracking-[0.1em]" style={{ color: GOLD }}>Compare all 60+ programmes <span className="transition-transform duration-300 group-hover:translate-x-1">→</span></a>
          </div>
        </div>
      </section>
    </main>
