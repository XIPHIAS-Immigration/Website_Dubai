...
// GoldenVisaHub — VARIANT ① "Hub": the consistent navy/gold idiom matching the
// citizenship / residency hubs (CountryHub / VerticalHub). Self-contained sample.
//
// Data source: the REAL golden-visa destinations — the 8 GOLDEN_VISA_SLUGS used by
// src/app/(site)/golden-visa/page.tsx (portugal, greece, uae, malta, cyprus,
// hungary, latvia, mauritius), each loaded by getResidencyCountries() there.
// Images: REAL full-bleed frames via countryImage() from
// src/components/Countries/country-image.ts. Each card links to its real route
// /residency/[slug]; the hero image is countryImage("uae").
...
// REAL golden-visa destinations (GOLDEN_VISA_SLUGS) with real key stats + real routes.
const DESTINATIONS = [
  { name: "Portugal", slug: "portugal", region: "Europe", stat: "From €250K" },
  { name: "Greece", slug: "greece", region: "Europe", stat: "From €250K" },
  { name: "United Arab Emirates", slug: "uae", region: "Africa & Middle East", stat: "10-year visa" },
  { name: "Malta", slug: "malta", region: "Europe", stat: "From €150K" },
  { name: "Cyprus", slug: "cyprus", region: "Europe", stat: "From €300K" },
  { name: "Hungary", slug: "hungary", region: "Europe", stat: "From €250K" },
  { name: "Latvia", slug: "latvia", region: "Europe", stat: "From €60K" },
  { name: "Mauritius", slug: "mauritius", region: "Africa & Middle East", stat: "From $375K" },
...
export default function GoldenVisaHub({ serifClass }: { serifClass: string }) {
        ...
        <div className="relative z-10 mx-auto w-full max-w-6xl px-6 sm:px-12 lg:px-20">
          ...
          <Fade reduce={reduce} delay={0.9}>
            <p className="mt-7 max-w-xl text-[16px] leading-relaxed text-white/75">
              Eight headline residence-by-investment routes — Portugal, Greece, the UAE, Malta and more — compared by investment, timeline and the lifestyle, schooling and mobility they unlock.
            </p>
          </Fade>
          ...
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              ...
              <h2 className={`${serifClass} mt-5 text-[clamp(2.2rem,4.4vw,3.6rem)] font-medium`}>
                <Rise text="Eight Golden Visa destinations." reduce={reduce} />
              </h2>
            </div>
                ...
                <a href={`/residency/${x.slug}`} className="group block aspect-[4/5]">
                  <div className="relative h-full w-full overflow-hidden rounded-md transition-shadow duration-300 group-hover:shadow-[0_30px_60px_-30px_rgba(8,18,42,0.6)]">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-6 text-[#eef3fb]">
                      ...
                      <p className="mt-2 text-[12px] text-white/70">{x.stat}</p>
