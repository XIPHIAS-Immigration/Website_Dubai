...
export default function InsightsNews({ serifClass }: { serifClass: string }) {
  return (
    <main>
      <section className="relative isolate px-6 py-24 text-[#0c1f3f] sm:px-12 lg:px-20" style={{ background: "#f3f7fd" }}>
        <Badge>Info section · Insights & news (real articles)</Badge>
            ...
            <Fade>
              <a href={FEATURED.href} className="group block">
                <div className="relative aspect-[16/10] w-full overflow-hidden rounded-lg">
                  <Image src={FEATURED.img} alt={FEATURED.title} fill sizes="(min-width:1024px) 40rem, 100vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
                ...
                <h3 className={`${serifClass} mt-5 text-[clamp(1.6rem,2.6vw,2.2rem)] font-medium leading-tight transition-colors group-hover:text-[#bfa15c]`}>{FEATURED.title}</h3>
                <p className="mt-3 max-w-xl text-[15px] leading-relaxed text-[#0c1f3f]/65">{FEATURED.excerpt}</p>
                <p className="mt-4 text-[12px] uppercase tracking-[0.14em] text-[#0c1f3f]/45">{FEATURED.date} · {FEATURED.read}</p>
              </a>
            </Fade>
