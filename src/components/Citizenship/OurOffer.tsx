...
import { Reveal, SandReveal, ShinyText, Magnetic } from "@/components/motion";
  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        ...
        {/* readable gold quote-mark watermark */}
        <span className="absolute bottom-2 end-4 font-sora text-[7rem] leading-none font-bold text-gold_deep/20 select-none">
          ✦
        </span>
      </div>
      ...
      <div className={`relative ${alignCls}`}>
        ...
        {subtitle ? (
          <Reveal y={12} delay={0.08}>
            <p
              className="mt-2 text-[15px] leading-7 text-ink/60"
        ...
        >
          {bullets.map((b, i) => (
            <li
              key={b}
              className="flex items-start gap-2 rounded-xl border border-gold/45 bg-sand/40 px-3 py-2 transition-colors hover:border-gold/65"
...
function Chip({ children }: { children: React.ReactNode }) {
  return (
    <li className="rounded-full border border-gold/45 bg-sand/40 px-3 py-1 text-xs text-ink/70">
      {children}
    </li>
  );
}
