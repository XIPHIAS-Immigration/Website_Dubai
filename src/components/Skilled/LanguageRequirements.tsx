    ...
    >
      ...
      <div className="px-4 sm:px-5 py-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-gold/45 bg-sand/40">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold font-sora text-ink">Language requirements</h3>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {hasMin ? (
            <span className="inline-flex items-center rounded-md bg-sand/50 text-ink/70 ring-1 ring-gold/40 px-2.5 py-1 text-xs">
              <CheckCircle2 className="mr-1 h-3.5 w-3.5 text-gold" aria-hidden />
              Minimum level: <span className="ml-1 font-semibold text-gold">{language!.minLevel}</span>
            </span>
          ) : null}
        </div>
      </div>
      ...
      <div className="px-4 sm:px-5 py-4 text-sm">
        {hasTests ? (
          <div className="mb-3">
            <div className="text-[10px] uppercase tracking-[0.14em] text-ink/40 mb-1">Accepted tests</div>
            <div className="flex flex-wrap gap-1.5">
              {tests.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center rounded-md bg-sand/50 text-ink/70 ring-1 ring-gold/10 px-2 py-1 text-xs font-medium"
                >
                  {t}
                </span>
              ))}
        ...
        {/* Helper note */}
        <p className="mt-1 text-[12px] leading-5 text-ink/45 flex items-start gap-1.5">
          <Info className="h-3.5 w-3.5 mt-0.5 shrink-0 text-gold" aria-hidden />
