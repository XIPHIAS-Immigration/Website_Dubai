    ...
    >
      ...
      <div className="px-4 sm:px-5 py-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-gold/45 bg-sand/40">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold font-sora text-ink">Points breakdown</h3>
          <span
            className="inline-flex items-center gap-1 text-[11px] uppercase tracking-[0.14em] text-ink/45"
        ...
        <div className="flex flex-wrap items-center gap-2">
          {typeof totalMax === "number" && totalMax > 0 ? (
            <span className="inline-flex items-center rounded-md bg-sand/50 text-ink/70 ring-1 ring-gold/10 px-2.5 py-1 text-xs">
              Max possible: <span className="ml-1 font-medium tabular-nums text-ink">{totalMax}</span> pts
            </span>
          ) : null}

          {typeof threshold === "number" ? (
            <span className="inline-flex items-center rounded-md bg-sand/50 text-ink/70 ring-1 ring-gold/40 px-2.5 py-1 text-xs">
              <Target className="mr-1 h-3.5 w-3.5 text-gold" aria-hidden />
              Pass mark: <span className="ml-1 font-semibold tabular-nums text-gold">{threshold}</span> pts
            </span>
          ) : null}
        </div>
      </div>
      ...
      {totalMax > 0 ? (
        <div className="px-4 sm:px-5 pt-3">
          <div className="relative h-2 w-full rounded-full bg-sand ring-1 ring-gold/10 overflow-hidden">
            <div
              className="absolute left-0 top-0 h-full bg-gold/30"
              style={{ width: "100%" }}
              aria-hidden
            />
          ...
          <div className="mt-1 mb-2 text-[11px] text-ink/45">
      ...
      <div className="hidden md:block">
        <table className="w-full text-sm">
          <thead className="text-left text-[10px] uppercase tracking-[0.14em] text-ink/45">
            <tr>
            ...
            {rows.map((r, i) => {
              const pct =
                totalMax > 0 && typeof r.max === "number"
                  ...
                  : 0;
              return (
                <tr key={`${r.category}-${i}`} className="hover:bg-white/[0.03]">
                  <td className="px-4 sm:px-5 py-3 font-medium text-ink">
                    <div className="flex items-center gap-3">
                      <div className="hidden h-2 w-20 rounded-full bg-sand md:block">
                        <div
                  ...
                  <td className="px-4 sm:px-5 py-3 text-right tabular-nums text-gold">
                    {typeof r.max === "number" ? r.max : "—"}
                  </td>
                  <td className="px-4 sm:px-5 py-3 text-ink/55">{r.notes ?? "—"}</td>
                </tr>
              );
            })}
          </tbody>
        ...
        return (
          <div key={`${r.category}-${i}`} className="p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm tabular-nums text-gold">{typeof r.max === "number" ? `${r.max} pts` : "—"}</div>
            </div>
            <div className="mt-2 h-2 w-full rounded-full bg-sand">
              <div
                className="h-2 rounded-full bg-gold/60"
                style={{ width: `${pct}%` }}
                aria-hidden
            ...
            {r.notes ? (
              <p className="mt-2 text-xs text-ink/55 leading-5">{r.notes}</p>
            ) : null}
          </div>
        );})}
      </div>
